...
function animateValue(id, start, end, duration) {
    ...
    function step(timestamp) {
        ...
        obj.innerText = "₹" + Math.round(start + range * progress);
...
function calculate() {
    ...
    setTimeout(() => {
        ...
        let percentage = expense !== 0 ? (profit / expense) * 100 : 0;
...
function updateGoal() {
    ...
    document.getElementById("goalText").innerText = "Progress: " + percent.toFixed(1) + "%";
...
function generateInsights() {
    ...
    if (highestCategory) {
        ...
        insightBox.innerHTML = "💡 Financial insights will appear here";
    }
}
...
function generateCalendar() {
    ...
    data.forEach(item => {
        ...
        dailyTotals[date] += Number(item.expense);
    });
    ...
    for (let i = 1; i <= 30; i++) {
        ...
        dayBox.innerHTML = `${dateKey.getDate()}<br>₹${value}`;

        if (value > 3000) dayBox.classList.add("high");
        else if (value > 1000) dayBox.classList.add("medium");
        else if (value > 0) dayBox.classList.add("low");
